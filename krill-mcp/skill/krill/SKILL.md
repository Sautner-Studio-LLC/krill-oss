...
Internalize this model before wiring anything — it drives every authoring decision:

- **Nodes are independent observers.** Every node lists the nodes it watches in `meta.sources`. When a source completes its work, every node observing it is **invoked** (woken) — there is no "parent executes its children" push path. The observed node knows nothing about its observers; **wiring always lives on the observer side**.
- **Invocation is a pull.** A woken node reads what it needs *at that moment* from its `meta.inputs` — the nodes whose last result (`meta.snapshot`) it consumes. `sources` decide *when* a node runs; `inputs` decide *what values* it reads. A node consuming another's output usually lists it in **both**.
- **Every node publishes its result in its own `meta.snapshot`.** A Calculation stores its computed value there; a webhook stores the HTTP response; a DataPoint stores its current reading. Downstream observers pull from there.
...
- **Verbs cascade.** When a node fires it sends its `meta.nodeAction` verb downstream — `EXECUTE` (run your normal action) or `RESET` (stand down / clear). Observers apply the *source's* verb, so one RESET upstream clears a whole chain. Nodes with no sensible response to a verb safely ignore it (best effort).
- **Parent/child is visual organization only.** The tree groups nodes in the UI; it carries no execution semantics. As a convenience, the server wires a newly created node's parent into its `sources` (+ `SOURCE_INVOKED`) when `sources` is empty and neither side is a Project/Server container — so a child observes its parent by default. Rewire freely with `set_node_wiring`.
...
4. Build **top-down, parent-first**. Each `create_node` call returns the new `nodeId`; use that as the `parent` for its children. Example chain: `KrillApp.DataPoint` on the server → `KrillApp.Trigger.HighThreshold` on the DataPoint → `KrillApp.Executor.OutgoingWebHook` on the Trigger. Remember the parent is only the default *source* — the tree is organization, the wiring is flow.
5. Overlay type-specific fields via the `meta` argument — e.g. `{"dataType": "DOUBLE", "unit": "°C", "precision": 1}` for a temperature DataPoint, `{"snapshot": {"timestamp": 0, "value": "100"}}` for a HighThreshold (trigger/filter thresholds live in `meta.snapshot.value` — TriggerMetaData and FilterMetaData have no separate `value` field). Unknown keys are silently dropped by the server (`ignoreUnknownKeys = true`), so extras are safe but typos go unnoticed — stick to the field names in the MetaData classes. If a meta field can't be determined at creation time, create the node first and then call `update_node {id, meta: {expression: "*/5 * * * * *"}}` to set it — the update propagates to connected clients via SSE so the UI reflects the change live.
6. **Wire the flow.** The server's creation default (parent → child's `sources`) covers straight chains. For anything else — a Calculation's formula variables, a DataPoint storing an executor's output, cross-branch or cross-server observation — call `set_node_wiring` on the **observer** with `sources` / `inputs` / `invocationTriggers`. A Calculation's `formula` references its `inputs` with bracket tokens `[<hostId>:<nodeId>]` (the NodeIdentity string form); every referenced node must be in `inputs`.
7. **Verify with `get_node`** after each create. The `create_node` response echoes what was sent, not what persisted; a round-trip read is the only ground truth. The server also **fills in defaults for meta fields you omitted** (e.g. a DataPoint you posted without a `snapshot` comes back with `snapshot: {timestamp: 0, value: ""}`) and **applies the parent-as-source default** — check `meta.sources` / `meta.invocationTriggers` on the read-back to see the wiring you actually got.
...
2. Call `record_snapshot` with either `{id, value, timestamp?}` for a single reading or `{id, snapshots: [{timestamp, value}, ...]}` for a series. `timestamp` is epoch **milliseconds**. (`id` is the DataPoint's UUID — same arg name as `get_node` / `read_series` / `delete_node`.) **If MCP tools aren't in-session**, snapshot writes have **no Krill REST endpoint** — don't probe `/data`, `/snapshot`, `/data/record`, etc. on `:8442` (they 404). Use the **Direct-to-MCP JSON-RPC fallback** in `references/mcp-tools.md` to reach `record_snapshot` through `http://<host>:50052/mcp`.
...
4. Each POST returns 202 Accepted before the server finishes ingesting. Two rules for verifying with `read_series`:
   - **Wait ~1.5 seconds before the first `read_series` call**, or be prepared to retry once. The ingest pipeline (`scope.launch` on the server) commonly returns 0 snapshots on an immediate follow-up read even on bare DataPoints with no filters — a short delay or one retry reliably recovers. Don't interpret an empty first read as "the write failed" unless the retry also returns empty.
   - **Persistence-verification is mandatory** when the DataPoint has a `DiscardAbove` / `DiscardBelow` / `Deadband` / `Debounce` filter wired into its `meta.inputs` — those can silently drop a snapshot for good. (Filters are evaluated from the DataPoint's `inputs`; a filter node merely parented under the DataPoint but absent from `inputs` does nothing.) On a bare DataPoint, verification is optional but still a useful round-trip.
