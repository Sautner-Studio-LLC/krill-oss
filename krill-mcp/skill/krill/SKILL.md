...
- **Registry is seed-config-driven, with lazy host registration.** The bootstrap registry comes from `/etc/krill-mcp/config.json`'s `seeds` array. Passing a previously-unknown `server` selector that contains a `.` (e.g. `pi-krill.local`, `192.168.1.10`) or an explicit `:port` triggers a one-shot dial of `https://<host>:<port>/health` with the swarm bearer; if it answers, the server is registered for the rest of this process's lifetime and the tool call proceeds — no `systemctl restart` needed. Bare names without a dot or port still don't auto-register (they collide with the UUID id space). Failures surface as `host unreachable: <selector>`. Cross-swarm transitive autodiscovery (one seed → enumerate every peer) is still pending: `bsautner/krill#178` tracks the krill-server side of that.
