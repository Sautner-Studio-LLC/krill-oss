...
1. If MCP tools are available in this Claude session, call `list_servers` → `server_health` → `list_nodes` (with a `type` filter when the user's intent narrows it). Use `get_node` for specific nodes the user names. Use `read_series` only when historical values matter.
   - **When the user names a node by display name or surrounding context** (*"the Vivarium mister"*, *"living room lamp"*) — call `find_node` instead of breadth-firsting with `list_nodes`. It iterates every registered server, scores by name + parent path, and returns ranked `(serverId, nodeId)` candidates with normalized scores. Fire on a top score `>= 0.9` that's clearly above the runner-up; when two candidates are within ~0.1 of each other, ask the user which one before mutating.
