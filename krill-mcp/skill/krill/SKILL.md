...
3. **Pick the parent project.** Diagrams must be children of a `KrillApp.Project` container.
   ...
   - **Project names are not swarm-unique** — see "Topology, auth, limits" below. When `list_servers` shows more than one server, call `list_projects` per-server (`server: "<id|host>"`) and treat each project as `(serverId, projectId)`. If two servers each host a project with the user's chosen name (e.g. both `pi-krill.local` and `pi-krill-05.local` have a `Vivarium`), **ask the user which server's project** before creating the diagram — never silently pick the first match or merge them.
...
### For "fire a Trigger or Executor manually"

Krill nodes are event-driven — a `KrillApp.Executor.LogicGate`, a `KrillApp.Trigger.*`, or a similar action node normally fires when its source DataPoint changes. To **manually** fire one (e.g. test a gate, or toggle a `KrillApp.Server.Pin` via a NOT-gate child of that pin) — the in-app equivalent of the manual-execute button:

1. **If `execute_node` is available** (tracked in [`bsautner/krill-oss#24`](https://github.com/bsautner/krill-oss/issues/24)) — call it on the gate's nodeId and verify by `read_series` on `meta.targets[0].nodeId` (or `get_node` for non-DataPoint targets like a Pin). This is the canonical recipe; prefer it whenever the tool is in the registry.
2. **If `execute_node` isn't available yet** — fall back to the **`record_snapshot`-upstream stopgap**:
   1. `get_node` on the gate to read `meta.sources[0].nodeId` — the upstream DataPoint that drives the evaluation.
   2. `record_snapshot` to that DataPoint with a value that flips the evaluation (e.g. write `1` then `0` to a DIGITAL source to toggle a NOT gate; cross a threshold for a `HighThreshold` trigger). Use the `record_snapshot` recipe above — same async + filter caveats apply.
   3. Verify by `read_series` on `meta.targets[0].nodeId`, or `get_node` for a Pin / non-time-series target, after the ~1.5 s ingest delay.

   **Caveats — call these out to the user before using the stopgap:**
   - It **pollutes the upstream DataPoint's history** with a synthetic reading that has no real-world source. Anyone reading the time-series later will see a bogus event.
   - It **doesn't work** for gates whose source is a derived/computed value with no DataPoint to write to (e.g. a gate fed by another gate's result, or a peer-prefixed cross-server source the local MCP can't write to).
   - Treat this as a stopgap that goes away once `execute_node` lands. Don't promote it to canonical guidance in your reply — frame it as "until the manual-execute primitive ships, here's the workaround."
...
- **Project (and node) names are not swarm-unique.** Two Krill servers in the same swarm can each host a project named "Vivarium" — neither the server nor the MCP enforces uniqueness across peers. Always treat `(serverId, projectId)` as the project's identity; the name is a label, not a key. When `list_projects` (or `list_nodes`) returns same-named matches across multiple registered servers, **ask the user which server** before acting — never silently pick the first one and never merge them. Pass `server:` explicitly on every follow-up tool call so the resolution stays unambiguous. This becomes the common case once transitive peer discovery ([`bsautner/krill-oss#23`](https://github.com/bsautner/krill-oss/issues/23)) ships and a single MCP routinely sees multiple servers.
