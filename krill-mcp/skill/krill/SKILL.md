...
- **v0.0.8 write surface covers any node type, DataPoint value writes, delete, and node action.** Project/Diagram helpers: `create_project`, `list_projects`, `create_diagram`, `update_diagram`, `get_diagram`, `upload_diagram_file`, `download_diagram_file`. Generic node authoring: `create_node`, `list_node_types`. DataPoint time-series writes: `record_snapshot`. Cascading delete: `delete_node` (deletes a Project recursively removes Diagrams/TaskLists/Journals/Cameras; deleting a DataPoint recursively removes its Filters/Triggers/Executors/Graphs). **Node action verb:** `set_node_action` sets `meta.nodeAction` to `EXECUTE` (default forward execution) or `RESET` (revert targets to initial state) on any Trigger, Executor, or TaskList node; read the current value via `get_node` → `meta.nodeAction`.
