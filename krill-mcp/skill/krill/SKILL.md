...
### For "fire this trigger / executor once" (manual run)

User intent looks like: *"run the lambda for me"*, *"toggle the gate now"*, *"fire the cron immediately"*, *"trigger this webhook"*. The right primitive is `execute_node` — not `record_snapshot` on the upstream DataPoint, even though that pathway can sometimes drive the chain.

1. Identify the target node — usually a `KrillApp.Executor.*` (LogicGate, Lambda, Calculation, Compute, SMTP, OutgoingWebHook) or `KrillApp.Trigger.*` (Button, CronTimer, etc.). Use `list_nodes` with a `type` filter, or `get_node` if the user named it.
2. Call `execute_node` with `{server?, id}`. The tool POSTs the existing node body back with `state="EXECUTED"`; the server's per-type processor reacts and runs the action.
3. The response includes the post-fire `node` GET — for actions whose effect lands on a *target* node (a LogicGate flipping a Pin's state, a Lambda recording a snapshot), follow up with `get_node` on `meta.targets[0].nodeId` after ~1s. The fired node itself just goes back to `state=EXECUTED`; the visible change is on the target.
4. **Don't substitute `record_snapshot`** for a fire request even when the gate's source happens to be a DataPoint. It pollutes the DataPoint's history with a synthetic value, won't work for executors whose source is a derived/computed value, and is the wrong abstraction — the user said "fire", not "fake a reading". Issue `bsautner/krill-oss#24` documents this confusion explicitly.
5. **What `execute_node` will refuse:** `KrillApp.Server`, `KrillApp.Client`, `KrillApp.Client.About`, `KrillApp.Server.Peer`, `KrillApp.Server.Backup`. These are containers / infrastructure; firing is meaningless. The tool returns an error — mutate a child instead (e.g. fire the Trigger under a Server, restore via the backup-restore flow rather than firing the Backup node).
...
- **v0.0.8 write surface covers any node type, DataPoint value writes, manual fire, and delete.** Project/Diagram helpers: `create_project`, `list_projects`, `create_diagram`, `update_diagram`, `get_diagram`, `upload_diagram_file`, `download_diagram_file`. Generic node authoring: `create_node`, `list_node_types`. DataPoint time-series writes: `record_snapshot`. Manual fire-once for Triggers/Executors: `execute_node`. Cascading delete: `delete_node` (deletes a Project recursively removes Diagrams/TaskLists/Journals/Cameras; deleting a DataPoint recursively removes its Filters/Triggers/Executors/Graphs).
