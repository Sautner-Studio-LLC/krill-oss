    ...
    private val TYPE_TABLE: List<KrillNodeType> = listOf(
        ...
        KrillNodeType(
            ...
            defaultMeta = meta(
                ...
                "invocationTriggers" to JsonArray(emptyList()),
                "nodeAction" to JsonPrimitive("EXECUTE"),
            ),
            role = "state",
            sideEffect = "low",
            ...
            notes = "`dataType` ∈ {TEXT, JSON, DIGITAL, DOUBLE, COLOR}. Use `record_snapshot` to write values. " +
                "Wire as a source via `set_node_wiring` — subscribers with SOURCE_INVOKED will fire on each new snapshot.",
            metaFieldHints = mapOf(
                ...
                "invocationTriggers" to "List<enum: SOURCE_INVOKED | ON_CLICK>",
                "nodeAction" to NODE_ACTION_HINT,
            ),
        ),
        KrillNodeType(
