/**
 * HTTP client class that talks to a Krill server's REST surface
 * (`/health`, `/node/<id>`, `/nodes`, `/data/series`, `/camera/...`,
 * `/backup/...`, etc.). One instance is typically registered with Koin
 * and shared across the application.
 *
 * Compared to the original `/shared` definition, this SDK form takes the
 * Ktor [io.ktor.client.HttpClient] as a constructor argument rather than
 * pulling it from the `httpClient` `expect` val — that lets the SDK be
 * agnostic about which platform engine the caller wires up (CIO on JVM,
 * OkHttp on Android, Darwin on iOS, JS on wasmJs) and also makes the class
 * unit-testable with a mock engine.
 */
 ...
 *   per [krill.zone.shared.security.PinDerivation.deriveBearerToken]) on
 *   each request. `null` skips the `Authorization` header entirely, which
...
class NodeHttp(
    ...
    /**
     * Returns the data-export URL for a node, optionally filtered by time
     * range. Caller composes a download request from the returned URL.
     */
    @OptIn(ExperimentalUuidApi::class)
