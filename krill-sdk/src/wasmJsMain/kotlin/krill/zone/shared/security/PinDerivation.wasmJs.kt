/**
 * wasmJs actual of [PinDerivation].
 *
 * The Web Crypto API is async-only and not yet exposed via Kotlin/Wasm
 * interop, and `javax.crypto` does not exist in the browser, so this
 * implementation falls back to a pure-Kotlin HMAC-SHA256 / PBKDF2. That is
 * acceptable because:
 *
 *  * the wasmJs client is typically served by the Krill server itself and the
 *    bearer token is injected via a `<meta>` tag — derivation happens once
 *    during FTUE PIN entry on non-kiosk WASM,
 *  * the inputs are short (a 6-digit PIN, a UUID), so a software SHA-256 is
 *    fast enough not to be user-visible.
 *
 * The numeric values come from FIPS 180-4 (SHA-256 round constants and IV).
 */
