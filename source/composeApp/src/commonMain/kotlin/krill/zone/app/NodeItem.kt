...
fun BoxScope.NodeItem(
    ...
    fun showMenu() {
        ...
        when {
            ...
            node.type is KrillApp.DataPoint.Graph || node.type is KrillApp.Project.Diagram || node.type is KrillApp.Project.Camera || node.type is KrillApp.Project -> {
                        ...
                        while (true) {
                            ...
                            try {
                                if (event.type == PointerEventType.Press && event.buttons.isSecondaryPressed) {
                                    ...
                                    if (node.type is KrillApp.Server.LLM || node.type is KrillApp.Project.Diagram || node.type is KrillApp.Project.Camera || node.type is KrillApp.Project) {
                                        screenCore.executeCommand(MenuCommand.Update)
                                    }
